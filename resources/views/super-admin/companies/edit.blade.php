...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-light dark:bg-dark rounded-base shadow-xs border border-default p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-heading dark:text-white">Edit Company</h1>
                <p class="text-body dark:text-gray-400 mt-1">Update company details</p>
            </div>
            ...
            <form action="{{ route('super-admin.companies.update', $company) }}" method="POST"
                ...
                <!-- Logo -->
                <div class="mb-4">
                    <label class="block text-sm font-medium text-heading dark:text-white mb-1">Current Logo</label>
                    @if($company->logo)
                        <div class="mb-2">
                    ...
                    @else
                        <p class="text-sm text-body mb-2">No logo uploaded</p>
                    @endif
                    <label for="logo" class="block text-sm font-medium text-heading dark:text-white mb-1">Upload New Logo
                        (Optional)</label>
                ...
                <div class="mb-4">
                    ...
                    <div class="space-y-3">
                        <input type="text" name="contact_info[email]" placeholder="Email"
                            value="{{ old('contact_info.email', $company->contact_info['email'] ?? '') }}"
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <input type="text" name="contact_info[phone]" placeholder="Phone"
                            value="{{ old('contact_info.phone', $company->contact_info['phone'] ?? '') }}"
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <input type="text" name="contact_info[address]" placeholder="Address"
                            value="{{ old('contact_info.address', $company->contact_info['address'] ?? '') }}"
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="status" value="1" {{ old('status', $company->status) ? 'checked' : '' }} class="rounded border-default text-primary-600 focus:ring-primary-500">
