...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-light dark:bg-dark rounded-base shadow-xs border border-default p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-heading dark:text-white">Edit Measurement</h1>
                <p class="text-body dark:text-neutral-400 mt-1">Campaign: {{ $campaign->title }}</p>
            </div>

            <form action="{{ route('items.update', [$campaign, $item]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="assigned_measurer_id"
                            class="block text-sm font-medium text-heading dark:text-white mb-1">Assigned Measurer</label>
                        <select name="assigned_measurer_id" id="assigned_measurer_id"
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                            <option value="">None</option>
                            @foreach($measurers as $measurer)
                                <option value="{{ $measurer->id }}" {{ old('assigned_measurer_id', $item->assigned_measurer_id) == $measurer->id ? 'selected' : '' }}>
                                    {{ $measurer->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label for="assigned_installer_id"
                            class="block text-sm font-medium text-heading dark:text-white mb-1">Assigned Installer</label>
                        <select name="assigned_installer_id" id="assigned_installer_id"
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                            <option value="">None</option>
                            @foreach($installers as $installer)
                                <option value="{{ $installer->id }}" {{ old('assigned_installer_id', $item->assigned_installer_id) == $installer->id ? 'selected' : '' }}>
                                    {{ $installer->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="status" required
                        ...
                        @foreach ($statusArr as $status)
                            <option value="{{ $status }}" {{ old('status', $item->status) == $item->status ? 'selected' : '' }}>
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        <label for="width" class="block text-sm font-medium text-heading dark:text-white mb-1">Width <span
                                class="text-red-500">*</span></label>
                        <input type="number" step="0.01" name="width" id="width" value="{{ old('width', $item->width) }}"
                            required
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.01" name="height" id="height"
                            value="{{ old('height', $item->height) }}" required
                            class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    </div>
                    <div>
                ...
                <div class="mb-4">
                    <label for="text" class="block text-sm font-medium text-heading dark:text-white mb-1">Text
                        (Arabic)</label>
                    <input type="text" name="text" id="text" value="{{ old('text', $item->text) }}"
                        class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="2"
                        class="w-full px-3 py-2 border border-default rounded-base bg-light dark:bg-dark text-heading dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">{{ old('notes', $item->notes) }}</textarea>
                </div>
